...
var validators = {
    ...
    required_nfs_volumes: createValidateArrayFn('required_nfs_volumes', {
        re: VOLUME_NAME_RE
    }),
...
function createValidateArrayFn(field, options) {
    ...
    if (options) {
        assert.optionalRegexp(options.re, 'options.re');
        assert.optionalFunc(options.validationFunction,
            'options.validationFunction');
    }

    return function (params) {
        var errs = [];
        var values = params[field];
        var hasInvalidValue;
        ...
        if (values !== undefined) {
            if (!Array.isArray(values)) {
                ...
                if (options && options.re) {
                    hasInvalidValue = values.some(function doesNotMatch(value) {
                        return !VOLUME_NAME_RE.test(value);
                    });

                    if (hasInvalidValue) {
                        errs.push(errors.invalidParamErr(field,
                            'Contains a value that does not match: '
                                + options.re));
                    }
                }

                if (options && options.validationFunction) {
                    hasInvalidValue = values.some(function invalidValue(value) {
